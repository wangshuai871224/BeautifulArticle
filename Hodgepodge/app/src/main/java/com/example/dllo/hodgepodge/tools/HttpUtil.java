...
public class HttpUtil {
    ...
    // 画报一级界面数据
    public static <T> void getRequest(String url, Class<T> mClass, NetCallBack netCallBack) {
        OkHttpManager.getInstance().get(url, mClass, netCallBack);
    }

    // 画报二级页面拼接
    public static <T> void connectRequest(int page, Class<T> mClass, NetCallBack netCallBack) {
